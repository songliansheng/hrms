...
import dev.songliansheng.hrms.datarest.dept.DeptRepo;
import dev.songliansheng.hrms.datarest.job.Job;
import dev.songliansheng.hrms.datarest.job.JobRepo;
...
import dev.songliansheng.hrms.datarest.user.*;
...
public class CustomRepoController {
    ...
    @PutMapping("empls/{id}")
    public @ResponseBody Empl updateEmpl(@RequestBody Empl empl){
        if(empl.getJob().getJname().equals(""))
            empl.setJob(null);
        empRepo.save(empl);
        return empl;
